...
export default function ProductForm({ initialData, isEditing = false }: ProductFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(parsedPrice) || parsedPrice < 29) {
      alert("Price must be at least 29 ₹. We only sell premium, high-value items.");
      setLoading(false);
      return;
    }
      ...
      <div className="grid lg:grid-cols-3 gap-10">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-8 space-y-6">
            <div>
              ...
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-4 text-white placeholder:text-stone-700 focus:border-gold outline-hidden transition-all text-lg font-medium"
                required
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-4 text-white placeholder:text-stone-700 focus:border-gold outline-hidden transition-all resize-none leading-relaxed"
              />
            </div>
          </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus-within:border-gold transition-all">
                ...
                <input
                  ...
                  className="bg-transparent border-none outline-hidden text-sm w-full text-white placeholder:text-stone-700"
                />
              </div>
              {formData.image && (
        ...
        <div className="space-y-8">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-8 space-y-8">
            <div>
              <label className="text-[10px] font-bold uppercase tracking-[0.2em] mb-4 block text-gold">Pricing</label>
              <div className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus-within:border-gold transition-all">
                ...
                <input
                  ...
                  min="29"
                  step="0.01"
                  className="bg-transparent border-none outline-hidden text-lg w-full text-white font-display font-bold"
                  required
                />
              </div>
            </div>
            ...
            <div className="pt-8 border-t border-white/10">
              <label className="text-[10px] font-bold text-stone-500 uppercase tracking-[0.2em] mb-4 block">Visibility</label>
              <label className="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/10 cursor-pointer hover:bg-white/8 transition-all group">
                ...
                <input
                  ...
                  className="w-5 h-5 rounded-sm border-white/10 bg-white/5 text-gold focus:ring-gold transition-all cursor-pointer"
                />
              </label>
